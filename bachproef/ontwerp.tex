...
Voor het aanmaken van een virtuele machine wordt een VDI-image gebruikt. Deze image wordt vooraf aangemaakt en bij het aanmaken van de VM gekoppeld als virtuele schijf.

\subsection{Linux Images}

Voor de Linux-images wordt een gebruiker \texttt{test} aangemaakt met wachtwoord \texttt{test}. De SSH-server moet actief zijn en verbinding via deze gebruiker moet mogelijk zijn via poort \texttt{22}. De gebruiker \texttt{test} beschikt over \texttt{sudo}-rechten om de provisioningsscripts uit te voeren.

\subsection{Windows Images}

Voor de Windows-images wordt eveneens een gebruiker \texttt{test} aangemaakt. Voor deze gebruiker wordt ingesteld dat het wachtwoord niet verloopt. De OpenSSH-server moet geïnstalleerd zijn en poort 22 moet openstaan. Daarnaast wordt het programma \texttt{PsExec} van PSTools gedownload, zodat scripts met administratorrechten kunnen worden uitgevoerd. Tot slot wordt \texttt{sysprep} uitgevoerd, zodat de Windows-VM herbruikbaar is. Zonder deze stap zouden conflicten kunnen optreden door identieke Security identifiers (SID) bij meerdere VM's.

\section{Provisioningworkflow}
...
\section{Netwerkadapters}

Voor de provisioner is enkel toegang voorzien tot de NAT-adapter, die gebruikt wordt voor het provisionen, en een optionele bridge-adapter. Deze twee netwerkadapters zijn de enige die beschikbaar zijn in RustProv. Ze zijn gekozen om de kernfunctionaliteit van een provisioner te ondersteunen. Er zijn geen extra netwerkadapters toegevoegd, omdat dit zou leiden tot feature creep: het toevoegen van extra functionaliteit die niet noodzakelijk is voor een proof of concept.
