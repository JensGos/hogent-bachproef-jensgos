...
In Hoofdstuk~\ref{ch:iteratieve-uitwerking} wordt het verloop van de ontwikkeling van RustProv beschreven aan de hand van de verschillende sprints.

In Hoofdstuk~\ref{ch:requirements-analyse} wordt de functionele en niet-functionele vereisten van RustProv geanalyseerd. Deze vereisten vormen de
basis voor de verdere ontwikkeling en evaluatie van de applicatie.

In Hoofdstuk~\ref{ch:toolselectie} wordt de selectie van de gebruikte tools en technologieën behandelt. Hierbij worden de relevante alternatieven besproken en wordt de uiteindelijke keuze gemotiveerd aan de hand van vooraf bepaalde criteria.

In Hoofdstuk~\ref{ch:ontwerp} wordt het ontwerp van de provisioner toegelicht, met aandacht voor de basisfunctionaliteit, de provisioningworkflow en de platformafhankelijke uitvoering.

In Hoofdstuk~\ref{ch:implementatie} wordt besproken hoe RustProv stapsgewijs werd opgebouwd en welke functionaliteiten per sprint werden toegevoegd.

In Hoofdstuk~\ref{ch:experimentopzet} wordt de opzet van de experimenten toegelicht, inclusief de testscenario's voor Windows en Linux.

In Hoofdstuk~\ref{ch:resultaten} worden de meetresultaten en de vergelijking met Vagrant besproken.
